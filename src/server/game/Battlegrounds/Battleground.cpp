...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (!(m_Events & BG_STARTING_EVENT_1))
    {
        ...
        SetStartDelayTime(StartDelayTimes[BATTLEGROUND_AB ? BG_STARTING_EVENT_FOURTH : BG_STARTING_EVENT_FIRST]);
        // First start warning - 2 or 1 minute
		if(!BATTLEGROUND_AB)
			SendMessageToAll(StartMessageIds[BG_STARTING_EVENT_FIRST], CHAT_MSG_BG_SYSTEM_NEUTRAL);

		if (!isArena() && !BATTLEGROUND_AB)
		{
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        else
        {
            ...
            // Announce BG starting
            if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE) && !BATTLEGROUND_AB)
